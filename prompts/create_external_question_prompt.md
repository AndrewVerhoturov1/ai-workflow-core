...
- Если вопрос затрагивает выбор между `/v1` и `/r1`, полезно явно дописать practical threshold:
  - `/v1` уместен, если достаточно одного prompt;
  - `/r1` нужен, если без `published links`, `task bundle`, `static manual` или `recorder-flow` вопрос теряет смысл.
...
- shorthand threshold:
  - достаточно одного prompt -> `/v1`;
  - нужен `published links/task bundle/static manual/recorder-flow` -> `/r1`.
