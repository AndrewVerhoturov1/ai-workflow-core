...
**Этот шаблон обязателен для каждого `/v1` prompt.** При явном shortcut `/v1` Codex **обязан** использовать этот шаблон. Prompt, написанный вручную без шаблона, не считается готовым `/v1` prompt-ом и не должен выдаваться пользователю. Перед выдачей prompt Codex выполняет `/v1` preflight-checklist (см. `AGENTS.md`).

Используй этот шаблон, когда человек вызвал `/v1` (или `/V1`, `/в1`, `/В1`) — prompt-only вопрос во внешний чат. См. также [`external_chat/manual.md`](../external_chat/manual.md) — обзор external chat workflow.

**Этот шаблон — только для `/v1`.** Для полного external launch package с published artifacts, task bundle и recorder используй [`prompts/create_external_chat_request.md`](create_external_chat_request.md) (маршрут `/r1`).
...
Кроме того, **каждый `/v1` prompt использует два слоя контекста**:

### Обязательные central links

Две ссылки из central core repo `AndrewVerhoturov1/ai-workflow-core`, обязательные в каждом `/v1` prompt:
...
Эти две central ссылки — минимальный контекст, без которого `/v1` prompt неполон. Обе обязательны.

Локальные версии этих файлов в consumer repo (`.ai/external_chats/external_chat_rules.md` и `.ai/repo_navigation.md`) — project-specific files. Они могут передаваться как дополнительные project-specific links, но не заменяют central core версии (`external_chat_rules.md` и `repo_navigation.md` в корне `ai-workflow-core`).
...
5. **`Candidate Navigation Entry`** — короткая выжимка (2–4 строки, один абзац) для будущей индексации в `V1_navigation.md` (project-local индекс в consumer repo). Это кандидат для последующего локального review, а не автоматическое обновление индекса.
...
- Если вопрос затрагивает выбор между `/v1` и `/r1`, полезно явно дописать practical threshold:
  - `/v1` — default external route, если достаточно одного prompt;
  - `/r1` нужен, если требуется хотя бы одно из: `published task bundle`, `strict traceability`, `recorder-ready capture`, `review published-artifact workflow`.
- Если вопрос касается consumer repo, prompt должен явно проговорить, что central navigation не даёт права делать claims о локальном repo без project-specific links/excerpts.
...
- `/v1` = default external route, один prompt сейчас.
- `/r1` = rare advanced/manual-only path, package + publish + record потом.
- shorthand threshold:
  - достаточно одного prompt и нет `/r1` threshold -> `/v1`;
  - нужен `published task bundle/strict traceability/recorder-ready capture/review published-artifact workflow` -> `/r1`.
...
Эскалируй вопрос в `/r1` (full external launch package; см. [`prompts/create_external_chat_request.md`](create_external_chat_request.md)), если:

- нужен published task bundle;
- нужна strict traceability published-artifact route;
- нужен recorder-ready capture через `kilo-recorder`;
- нужен review самого published-artifact / external-package workflow.
